...
class Kannisto:
    ...
    def __init__(self,
                 fit_ages: np.array = np.arange(60, 81, 5),
                 predict_ages: np.array = np.arange(85, 101, 5)):
        self.mx = None
        self.all_ages = None
        self.observed = None
...
class Brass():
    def __init__(self):
        self.observed_lx = None
        self.observed_x = None
        self.standard_lx = None
        self.standard_logit_lx_to_regr = None

    def fit(self, observed_lx, observed_x, standard_lx):
        self.observed_lx = np.array(observed_lx)
        self.observed_x = np.array(observed_x)
        self.standard_lx = np.array(standard_lx)
        return self

    def transform(self):
        self.logit_lx = self.logit(1 - self.observed_lx)
        self.logit_standard_lx = self.logit(1 - self.standard_lx)
        self.logit_standard_lx_to_regr = self.logit_standard_lx[self.observed_x]
        self.a, self.b = self.get_regr_coef()
        self.modeled_logit_lx = self.get_modeled_logit_lx()
        self.modeled_lx = self.reverse_logit(self.modeled_logit_lx)
        return self.modeled_lx

    @staticmethod
    def logit(x):
        logit_x = 0.5 * np.log(x / (1 - x))
        return logit_x

    @staticmethod
    def reverse_logit(logit_x):
        x = 1 / (1 + np.exp(2 * logit_x))
        return x

    def get_regr_coef(self):
        lr = LinearRegression()
        lr.fit(
            X=self.logit_standard_lx_to_regr.reshape(-1, 1),
            y=self.logit_lx
        )
        b = lr.coef_[0]
        a = self.logit_lx.mean() - b * np.average(self.logit_standard_lx_to_regr)
        return a, b

    def get_modeled_logit_lx(self):
        modeled_logit_lx = self.a + self.b * self.logit_standard_lx
        return modeled_logit_lx
